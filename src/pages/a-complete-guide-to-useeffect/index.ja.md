...
---
あなたはHooksを使っていくつかのコンポーネントを書いてきました。小さなアプリかもしれません。でもあなたは概ね満足しています。ここに至るまでにAPIを使いこなし、コツも少々つかんできました。繰り返し使われるロジックを抽出するためにcustom Hooks を作ったことだってあり（300行が消えました！）、それを同僚に見せびらかしもしました。彼らは「よくやったね」と言ってくれました。
...
しかし`useEffect`（副作用を使う）となると、それぞれどうもうまく噛み合いません。何か忘れてしまっているような、嫌な気持ちがしつこく残ります。これはまるでクラスのライフサイクルのようですが...本当にそうでしょうか？あなたはこういった質問をしていることに気がつくでしょう:

* 🤔 どうやって `componentDidMount` を `useEffect`で再現するのだろう？
* 🤔 どうやって `useEffect`内でただしくデータをフェッチするのだろう？`[]`って何だろう?
* 🤔 副作用の依存性として関数を指定する必要があるのだろうか？しなくてもよいのだろうか？
* 🤔 なぜ時々再フェッチの無限ループに陥るのだろう？
* 🤔 なぜ時々e副作用内で古いstateやpropの値を取得してしまうのだろう？
...
Hooksを使い始めた時、私もこういった疑問全てで混乱しました。最初のドキュメントを書いている時でさえ、これらの微妙な点のいくつかをはっきり把握はしていませんでした。それ以来、「アハ」体験が何度かあり、それをみなさんにシェアしたいです。**この深い掘り下げはこれらの疑問の答えを理解しやすくするでしょう**
...
答えを*よく見る*ために、私たちは一歩下がる必要があります。この記事の目的はあなたに箇条書きされたレシピのリストを与えることではありません。あなたが本当の意味で`useEffect`を理解することを助けることが目的なのです。新しく学ぶことはそれほど無いかもしれません。実際、ここからは知識を捨て去ること（アンラーニング）にほとんどの時間を費やしていきます。
...
私は、全てがまとまった後で初めて、馴染みあるクラスのライフサイクルメソッドのプリズムを通して`useEffect` Hookを見ることを止めました。
